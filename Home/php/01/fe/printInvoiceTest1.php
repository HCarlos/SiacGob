...
switch ($serie){
	...
	case "B":
			...
			$file_user         = "fiscalizacion@diariopresente.com.mx";
			$file_rfc          = "SIT010509694";
			$file_pass          = "SIT010509694";
...
$result = mysql_query("select importe, descuento, iva, total, forma_pago_desc, metodo_pago_desc, isfe, idcontrato, idcli, cuenta, idper from _viFacturasEncab where idfactura = $idfactura limit 1");
...
if ($folfis!=""){
	...
	mysql_select_db($Conn->db);
	mysql_query("SET NAMES UTF8");
	$result = mysql_query("update facturas_encab set isfe = 1, UUID = '$folfis', xml = '$fxml', pdf='$fpdf', folio = $folio, fecha = NOW(), fecha_timbrado = NOW() where idfactura = $idfactura ");
	mysql_close($mysql);
}
