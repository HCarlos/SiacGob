...
<div id="toolbar" class="ui-widget-header ui-corner-all">
	...
	<button id="printItemsCheck">Imprimir</button>
	<span id="spanTitle">Esquemar Publicidad</span>
</div>
...
<script type="text/javascript"> 
	...
	$("#printItemsCheck").on('click',function(event){
		event.preventDefault();
		printItemsCheck();
...
-->	
	...
	$( "#printItemsCheck" ).button({text: true,icons: {primary: "ui-icon-print"}});
...
function tablaDeElementos(responseText, statusText, xhr, $form){
	...
	$.post(obj.getValue(0)+"getEC/", { o:obj.index, t:16, p:4,c:q+"|"+oBy+orderBy,from:oPag.currentPage, cantidad:oPag.CantidadPagina },
		function(json){
			//alert(json.length);
...
function cargarRegistrosLeidos(json){
	...
	$.each(json, function(i, item) {
		...
		var id=item.description!=null?item.idcontratoesqval:null;
		var var1;
		var var2 = "";
		...
		if (item.description!=null) {
			//alert(item.description);
		    var1 = item.description.split("|");
		    var2 = var1[6];
		}
		isDisabled = item.description!=null?" disabled ":"";	
		isnulo = item.description!=null?" checked disabled ":"";	
		isdelitem = item.description!=null?'<a id="delCurrentItem-'+i+'" class="delCurrentItem delCurrentItemSpan"> Eliminar</a>':"";	
		...
		str +='<td class="prodc"><span>'+item.producto+" - "+item.guia+'</span></td>'; 
	...
	$(".delCurrentItem").on('click',function(event){
			...
			$.post(obj.getValue(0)+"getEC/", { o:obj.index, t:2, p:2, c:ind },
 				function(json){
				     if (json[0].msg=="OK"){
						sayMessage(json[0].msg,json[0].msg,$("#msgResponse"),"Dato eliminado con éxito",1000);
